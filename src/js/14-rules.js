...
function rules(w,o,h,p){
  ...
  if(EMPLOYER.includes(p)&&NO_EMPLOYER.includes(w))
    add("bad","x","There is no employer to invoice.",
  ...
  if(p===18&&!KIDS.includes(w))
    add("bad","x","Parents do not pay for working professionals.",
  ...
  if([5,6].includes(p)&&ORG_BUYER.includes(w))
    add("bad","x","Salary-linked pricing needs a salary you influence.",
  ...
  if(p===21&&![2,19].includes(h))
    add("bad","x","Expert networks buy hours, not products.",
  ...
  if(p===13&&MW[w][0]<4)
    add("warn","alert","Certification requires trust you do not have yet.",
  ...
  if(TRAVEL.includes(h))
    add("warn","alert","This breaks your stated goal of living anywhere.",
  ...
  if(CONTENT.includes(h)&&[0,1,3,4].includes(p))
    add("warn","alert","Content is not a purchase.",
  ...
  if(ORG_BUYER.includes(w)&&LEARNER.includes(p))
    add("warn","alert","You found the buyer with budget, then asked the individual to pay.",
  ...
  if(p===9)
    add("warn","alert","Ads and affiliate revenue cap you early.",
  ...
  if(o===13&&MP[p][0]>=4)
    add("warn","alert","Belonging is real, but it is hard to price high.",
  ...
  if(w===11&&EMPLOYER.includes(p))
    add("ok","check","This is the strongest payer structure available to you.",
  ...
  if(MH[h][2]===5&&MP[p][1]>=4)
    add("ok","check","Location-proof and fast to cash.",
  ...
  if(MO[o][3]===5&&MW[w][1]>=4)
    add("ok","check","This sits directly on your unfair advantage.",
  ...
  if(MH[h][4]===0)
    add("ok","check","You are not the product here.",
